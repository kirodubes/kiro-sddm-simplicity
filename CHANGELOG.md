...
- Keyboard: `import QtQuick.VirtualKeyboard 2.15` (from `qt5-virtualkeyboard`), a `property bool keyboardVisible`, and an `InputPanel` that slides up from the bottom. The VK plugin **must be the Qt5 build**: the package ships `etc/sddm.conf.d/999-force-x11` (`DisplayServer=x11`), so the live greeter is the **Qt5** `sddm-greeter`; `qt6-virtualkeyboard` lands in `/usr/lib/qt6/qml` and is invisible to it → "module not installed" → the greeter falls back to the embedded theme. The import is **explicitly versioned** (`2.15`, matching the other imports): an unversioned `import QtQuick.VirtualKeyboard` is rejected by the Qt5 parser with "library import requires a version", and `qt5-virtualkeyboard` provides exactly `2.15`. Visibility is gated on `keyboardVisible` (driven only by the toggle button) rather than `inputPanel.active`, so the keyboard is **toggle-only** and never auto-pops on field focus. Tab order weaves the new button between `shutdown` and `session`.
...
- The bottom power-button row (which holds the **Keyboard** toggle) likewise lifts above the keyboard when it's open — `bottomMargin: keyboardVisible ? inputPanel.height + 10 : 10` (200ms `Behavior`). Without this the full-width `InputPanel` (`z: 99`) covered its own toggle button, so once the keyboard was raised it could not be dismissed.
- The keyboard's own hide key now dismisses it too: a `Connections` on `Qt.inputMethod` clears `keyboardVisible` when the IM goes invisible. One-way (hide → close) only, so the panel still never auto-opens on field focus — toggle-only behaviour is preserved. Verified live: both the **Keyboard** button and the keyboard's hide key close it, opening is unaffected.
...
- Requires the **`qt5-virtualkeyboard`** package (the live greeter is the Qt5 `sddm-greeter` — see the Keyboard note above) and **`InputMethod=qtvirtualkeyboard` in `/etc/sddm.conf`** — NOT a `sddm.conf.d/` drop-in: per `sddm.conf(5)` (confirmed on hardware) `/etc/sddm.conf` outranks every drop-in, and it ships with an empty `InputMethod=` that would otherwise clobber a drop-in value. The package's `readme.install` writes `InputMethod` to `/etc/sddm.conf` directly on install, and blanks it on remove.
