...
- Keyboard: `import QtQuick.VirtualKeyboard 2.15` (from `qt6-virtualkeyboard` — the live greeter is Qt6 `sddm-git`), a `property bool keyboardVisible`, and an `InputPanel` that slides up from the bottom. The import is **explicitly versioned** (`2.15`, matching the other imports): an unversioned `import QtQuick.VirtualKeyboard` is rejected by a version-strict QML parser with "library import requires a version", and Qt6 accepts the `2.15` form just as it does for `QtQuick.Controls`. Visibility is gated on `keyboardVisible` (driven only by the toggle button) rather than `inputPanel.active`, so the keyboard is **toggle-only** and never auto-pops on field focus. Tab order weaves the new button between `shutdown` and `session`.
